...
func TestClassifyContent(t *testing.T) {
	tests := []struct {
		...
		{name: "generated marker at end", input: "package foo\nfunc main() {}\n// Code generated by protoc DO NOT EDIT", want: ContentGenerated},
	}
...
func TestMarkSuppressed(t *testing.T) {
	...
	if !files[0].Suppressed {
		t.Error("flow-state.json should be suppressed")
	}
	if files[1].Suppressed {
		t.Error("important.go should not be suppressed")
	}
	if !files[2].Suppressed {
		t.Error("scratch.tmp should be suppressed")
	}
...
func TestCheckCompleteness(t *testing.T) {
	...
	cr := CheckCompleteness(fullFiles)
	if cr.Score != 100 {
		t.Errorf("full repo score = %d, want 100", cr.Score)
	}
	if len(cr.Missing) != 0 {
		t.Errorf("full repo missing = %v, want empty", cr.Missing)
	}

	// Empty repo — missing everything.
	emptyCR := CheckCompleteness(nil)
	if emptyCR.Score >= 50 {
		t.Errorf("empty repo score = %d, want < 50", emptyCR.Score)
	}
	if len(emptyCR.Missing) == 0 {
		t.Error("empty repo should have missing items")
	}

	// Repo with just README and LICENSE.
	minFiles := []FileInfo{
		{RelPath: "README.md"},
		{RelPath: "LICENSE.md"},
	}
	minCR := CheckCompleteness(minFiles)
	if minCR.Score == 100 {
		t.Error("minimal repo should not score 100")
	}
	// Should still be missing .gitignore at least.
	hasGitignore := false
	for _, m := range minCR.Missing {
		if m.Name == ".gitignore" {
			hasGitignore = true
		}
	}
	if !hasGitignore {
		t.Error("minimal repo should be missing .gitignore")
	}
}
